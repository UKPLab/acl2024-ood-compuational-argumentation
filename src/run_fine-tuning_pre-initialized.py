...
from utils.training import check_run_done, get_pre_init_run, truncate_sentence


@click.command()
@click.option('--task', type=str, default="xnli")
...
def main(task, model_name, fold, setup, pooling, seed, steps, batch_size, epochs, dropout_rate, learning_rate, dump):
    ...
    training = "FINE_TUNING_PRE_INIT_SINGLE_STEP"
    ...
    if not is_run_done:
        ...
        pre_init_runs = get_pre_init_run(task, hyperparameter, steps=steps)

        if len(pre_init_runs) == 1:
            pre_init_run = pre_init_runs[0]
            model_artifact_dir = [ele for ele in list(pre_init_run.logged_artifacts()) if "model" in ele.name][0].download()
            frozen_model_head = FrozenClassification.load_from_checkpoint(model_artifact_dir + "/model_dump.ckpt", input_dim=768, num_classes=num_classes, dropout_rate=0.1, hyperparameter=hyperparameter)

            if hasattr(model, "classifier"):
                model.classifier.weight == frozen_model_head.classifier.weight
            else:
                children = list(model.base_model.children())

                while len(children) > 0:
                    last_child = children[-1]
                    children = list(last_child.children())

                last_child.weight = frozen_model_head.classifier.weight

            os.system("rm -rf " + model_artifact_dir)
